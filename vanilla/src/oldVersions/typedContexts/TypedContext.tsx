...
export function CalendarProvider({ children }: CalendarProviderProps) {
  ...
  const [events, setEvents] = useLocalStorage<EventsByDate>("eventsStoredData", {})
  ...
  function addEvent(date: Date, newEvent: CalendarEvent) {
    const key = format(date, GLOBAL_EVENT_KEY_DATE_FORMAT)
    setEvents((prev: object) => {
      const prevEventsForSelectedDate = prev[key] || []
      return { ...prev, [key]: [...prevEventsForSelectedDate, newEvent] }
    })
  }

  function deleteEvent(date: Date, index: number) {
    const key = format(date, GLOBAL_EVENT_KEY_DATE_FORMAT)
    setEvents((prev) => {
      const prevEventsForSelectedDate = prev[key] || []
  ...
  function updateEvent(date: Date, index: number, updatedEvent: CalendarEvent) {
    const key = format(date, GLOBAL_EVENT_KEY_DATE_FORMAT)
    setEvents((prev) => {
      const prevEventsForSelectedDate = prev[key] || []
